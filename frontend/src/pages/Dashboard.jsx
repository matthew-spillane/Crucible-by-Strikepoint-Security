...
const SCAN_COLUMNS = [
  ...
  { key: 'timestamp', label: 'Time',      render: (v) => <span className="font-mono text-xs text-text-muted">{format(v, 'HH:mm:ss')}</span> },
]

const FEED_TYPE = { finding: 'text-warning', scan: 'text-accent', info: 'text-text-muted' }
        ...
        <Card className="lg:col-span-2 flex flex-col min-h-[240px]">
          ...
          <div className="flex-1 flex flex-col gap-0 overflow-y-auto">
            {ACTIVITY_FEED.map((item, i) => (
              <div key={i} className="flex items-start gap-3 px-2 py-2.5 border-b border-border/50 last:border-0 hover:bg-surface-2/40 transition-colors">
                <span className="font-mono text-[10px] text-text-muted shrink-0 mt-0.5 w-16">{format(item.ts, 'HH:mm:ss')}</span>
                <span className={`font-mono text-xs leading-relaxed ${FEED_TYPE[item.type] || 'text-text-secondary'}`}>{item.msg}</span>
              </div>
            ))}
          </div>
        </Card>
